            ...
            <div className="rounded-lg border border-slate-100 p-2 h-24 relative overflow-hidden">
                ...
                <div className="absolute bottom-2 left-2 right-2 flex items-end gap-1 h-14">
                    {[40, 65, 30, 80, 55, 90, 70].map((h, i) => (
                        <div key={i} className="flex-1 bg-sky-200 rounded-t-sm" style={{ height: `${h}%` }} />
                    ))}
                </div>
            </div>
            <div className="rounded-lg border border-sky-100 bg-sky-50/50 p-2">
                <p className="text-xs font-black text-sky-700">Full Stack Development</p>
        ...
        <div className="space-y-2">
            ...
            {[
                ...
                <div key={g.task} className="flex items-center gap-2 rounded-lg border border-slate-100 px-2 py-1.5">
                    <span className={`w-3.5 h-3.5 rounded border-2 shrink-0 ${g.done ? 'bg-sky-600 border-sky-600' : 'border-slate-300'}`} />
                    <span className={`text-xs font-semibold ${g.done ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{g.task}</span>
                </div>
            ))}
            <div className="flex items-center justify-between rounded-lg bg-sky-50 border border-sky-100 px-2 py-1.5">
                <span className="text-xs font-bold text-sky-700">Current streak</span>
                <span className="text-sm font-black text-sky-600">7 days 🔥</span>
            </div>
        </div>
    </BrowserChrome>
);
...
const ResourcesPreview = () => (
    <BrowserChrome title="learning-tracker.app/bookmarks">
        <div className="space-y-2">
            ...
            {[
                { title: 'React Official Docs', tag: 'Documentation', color: 'text-sky-600 bg-sky-50' },
        ...
        <div className="space-y-2">
            ...
            {[
                ...
                <div key={t.label} className="flex items-center justify-between rounded-lg border border-slate-100 px-2 py-1.5">
                    ...
                    <span className={`w-8 h-4 rounded-full relative ${t.on ? 'bg-sky-600' : 'bg-slate-200'}`}>
