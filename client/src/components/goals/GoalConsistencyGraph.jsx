                ...
                <div className="inline-flex gap-2 min-w-full">
                    <div className="flex flex-col justify-between py-0.5 text-xs font-bold text-slate-400 dark:text-slate-500 shrink-0">
                        {DAY_LABELS.map((label, i) => (
                            <span key={label} className={`h-6 sm:h-7 leading-6 sm:leading-7 ${i % 2 === 0 ? '' : 'opacity-0 sm:opacity-100'}`}>
                                {label}
                            </span>
                        ))}
                    </div>
                                ...
                                {Array.from({ length: 7 }).map((_, rowIndex) => {
                                    ...
                                    if (!day) {
                                        return (
                                            <span
                                                key={rowIndex}
                                                className="w-6 h-6 sm:w-7 sm:h-7 rounded-md bg-transparent"
                                            />
                                        );
                                    }
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-6 h-6 sm:w-7 sm:h-7 rounded-md border transition-transform ${intensityClass(day)} ${isToday ? 'ring-2 ring-violet-400/70 dark:ring-violet-300/80' : ''} ${isHovered ? 'scale-110 ring-2 ring-violet-300 dark:ring-violet-500' : ''}`}
                                        />
                                    );
                                })}
                            </div>
            ...
            <div className="mt-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400">
                    ...
                    {[0, 1, 2, 3, 4].map((level) => (
                        <span
                            key={level}
                            className={`w-6 h-6 sm:w-7 sm:h-7 rounded-md border ${intensityClass({ count: level, allCompleted: level === 4 })}`}
                        />
                    ))}
                    <span>More</span>
                </div>
