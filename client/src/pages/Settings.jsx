        ...
        <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 pb-4 border-b border-slate-200 dark:border-slate-800">
                <div className="max-w-2xl">
                    ...
                    <p className="text-slate-700 dark:text-slate-400 mt-4 text-xl font-medium leading-relaxed">Customize your learning environment and notification behavior.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-1 space-y-10">
                    <div className="bg-white dark:bg-slate-900 premium-shadow rounded-3xl p-10 text-center relative overflow-hidden group border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            ...
                            <p className="text-sm font-bold text-slate-600 leading-relaxed italic px-4">{user?.bio}</p>
                        </div>
                        ...
                        <div className="mt-10 pt-10 border-t border-slate-50 dark:border-slate-800 space-y-3">
                             {['General', 'Security', 'Connectivity', 'Export Data'].map((item, idx) => (
                                <button key={idx} className={`w-full text-left px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${idx === 0 ? 'bg-violet-50 dark:bg-violet-900/30 text-violet-600' : 'text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-200 group'}`}>
                                    {item}
                                </button>
                            ))}
                        </div>
                ...
                <div className="lg:col-span-2 space-y-10">
                    ...
                    <div id="reminders" className="bg-white dark:bg-slate-900 premium-shadow rounded-3xl p-12 border border-slate-100 dark:border-slate-800">
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-10 flex items-center gap-5">
                            <div className="w-12 h-12 bg-rose-50 dark:bg-rose-900/40 text-rose-600 dark:rose-400 rounded-2xl flex items-center justify-center shadow-sm"><FiBell size={20} /></div>
                            ...
                            {[
                                ...
                                <div key={item.key} className="flex items-center justify-between">
                                     <div className="flex-1 pr-6">
                                        ...
                                        <p className="text-[10px] text-slate-600 dark:text-slate-400 font-bold mt-2 uppercase tracking-tight">{item.desc}</p>
                                    </div>
                                    <button 
