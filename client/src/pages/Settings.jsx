                            ...
                            {[
                                ...
                                <div key={item.key} className="flex items-center justify-between">
                                    <div className="flex-1 pr-6">
                                        <h4 className="text-xs font-black text-slate-900 tracking-widest uppercase">{item.label}</h4>
                                        <p className="text-[10px] text-slate-400 font-bold mt-2 uppercase tracking-tight">{item.desc}</p>
                                    </div>
                                    <button 
                                        onClick={() => handleToggle(item.key)}
                                        className={`w-14 h-8 rounded-full transition-all duration-500 relative shrink-0 ${notifications[item.key] ? 'bg-indigo-600 shadow-lg shadow-indigo-100' : 'bg-slate-200'}`}
                                    >
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-10">
                            ...
                            <div className="flex items-center bg-slate-50/80 rounded-[1.5rem] p-2.5 gap-2 border border-slate-100">
                                ...
                                <div className="flex gap-1.5 p-1 pr-2">
                                    <button 
                                        onClick={() => setAmPm('AM')}
                                        className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${amPm === 'AM' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600'}`}
                                    >
                                        AM
                                    </button>
                                    <button 
                                        onClick={() => setAmPm('PM')}
                                        className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${amPm === 'PM' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600'}`}
                                    >
                                        PM
                                    </button>
                                </div>
