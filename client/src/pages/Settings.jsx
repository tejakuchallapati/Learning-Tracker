        ...
        <div className="px-4 md:px-6 max-w-5xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
                <div className="max-w-xl">
                    ...
                    <p className="text-slate-700 dark:text-slate-400 mt-2 text-xs md:text-sm font-semibold leading-relaxed">Customize your learning environment and notification behavior.</p>
                </div>
                <div className="flex gap-4 shrink-0">
                    <button 
                        onClick={handleSave}
                        className="w-full sm:w-auto px-5 py-3.5 bg-slate-900 dark:bg-slate-800 text-white rounded-xl font-bold text-xs md:text-sm hover:bg-violet-600 transition-all shadow-md shadow-slate-200 dark:shadow-none flex items-center justify-center gap-2 btn-hover-scale uppercase tracking-wider"
                    >
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white dark:bg-slate-900 premium-shadow rounded-2xl p-6 relative overflow-hidden group border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="relative z-10 flex flex-col items-center text-center">
                            ...
                            <p className="text-[10px] md:text-xs font-black text-violet-500 uppercase tracking-[0.2em] mt-1.5 leading-none">{user?.role}</p>
                            <div className="h-px w-16 bg-slate-100 dark:bg-slate-800 my-4"></div>
                            <p className="text-xs md:text-sm font-semibold text-slate-700 dark:text-slate-300 leading-relaxed italic px-2">{user?.bio}</p>
                        </div>
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-50 dark:border-slate-800 flex flex-row overflow-x-auto gap-2 md:flex-col md:space-y-1.5 pb-2 scrollbar-none whitespace-nowrap">
                            {['General', 'Security', 'Connectivity', 'Export Data'].map((item, idx) => (
                                <button 
                                    key={idx} 
                                    className={`px-4 py-2.5 md:w-full md:text-left md:px-5 md:py-3.5 rounded-xl text-xs md:text-sm font-bold uppercase tracking-wider transition-all ${idx === 0 ? 'bg-violet-50 dark:bg-violet-900/30 text-violet-600' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-200'}`}
                                >
                                    {item}
                                </button>
                            ))}
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 premium-shadow rounded-2xl p-6 border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 pl-1">Handle</label>
                                <input 
                                    type="text" 
                                    value={formData.name} 
                                    onChange={e => setFormData({...formData, name: e.target.value})} 
                                    className="w-full h-12 bg-slate-50/50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 text-base font-semibold text-slate-900 dark:text-white focus:ring-4 focus:ring-violet-50 dark:focus:ring-violet-900/10 transition-all outline-none" 
                                />
                            </div>
                            <div className="space-y-1.5">
                            ...
                            <div className="md:col-span-2 space-y-1.5">
                                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 pl-1">Primary Email</label>
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full h-12 bg-slate-50/50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 pl-10 text-base font-semibold text-slate-900 dark:text-white focus:ring-4 focus:ring-violet-50 dark:focus:ring-violet-900/10 transition-all outline-none" 
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="reminders" className="bg-white dark:bg-slate-900 premium-shadow rounded-2xl p-6 border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 mb-6 relative overflow-hidden group">
                            ...
                            <h4 className="text-xs md:text-sm font-black text-slate-900 dark:text-white flex items-center gap-3 relative z-10"><FiActivity className="text-violet-600" /> SYNC PROTOCOL</h4>
                            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-2 font-semibold leading-relaxed max-w-lg relative z-10">Reminders are dispatched using local browser latency algorithms to maximize daily streak retention.</p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
                            <div className="flex-1 min-w-0">
                                <h4 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white tracking-wider uppercase">Adaptive Time Window</h4>
                                <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-1">Set your high-performance focus period.</p>
                            </div>
                            <div className="flex items-center bg-slate-50/80 dark:bg-slate-800 rounded-xl p-2 gap-2 border border-slate-200 dark:border-slate-700 shrink-0">
                                ...
                                <div className="flex gap-1">
                                    <button 
                                        ...
                                        className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${amPm === 'AM' ? 'bg-violet-600 text-white shadow-md shadow-violet-100/50' : 'text-slate-400 hover:text-violet-600'}`}
                                    >
                                        AM
                                    </button>
                                    <button 
                                        onClick={() => handleAmPmChange('PM')}
                                        className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${amPm === 'PM' ? 'bg-violet-600 text-white shadow-md shadow-violet-100/50' : 'text-slate-400 hover:text-violet-600'}`}
                                    >
                                        PM
                                    </button>
                                </div>
