        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    <h3 className="text-lg font-bold text-gray-800 mb-6">Smart Progress Analysis</h3>
                    <div className="space-y-6">
                        {analytics?.goalsAnalysis?.map(g => (
                            <div key={g.goalId}>
                                <div className="flex justify-between items-end mb-2">
                                    <span className="font-semibold text-gray-700">{g.technology}</span>
                                    <span className="text-xs text-gray-500">Expected: {g.expectedProgress}%</span>
                                </div>
                                <ProgressBar progress={g.currentProgress} label="Current" />
                                <p className="mt-2 text-sm text-gray-600 italic border-l-2 border-indigo-400 pl-3">
                                    "{g.suggestion}"
                                </p>
                            </div>
                        ))}
